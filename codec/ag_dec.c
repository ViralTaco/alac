...
/*	And on the subject of the CodeWarrior x86 compiler and inlining, I
   reworked a lot of this to help the compiler out.   In many cases this
   required manual inlining or a macro.  Sorry if it is ugly but the performance
...
void set_standard_ag_params(AGParamRecPtr params, uint32_t fullwidth,
  ...
  /* Use
          fullwidth = sectorwidth = numOfSamples, for analog 1-dimensional
     type-short data, but use fullwidth = full image width, sectorwidth = sector
     (patch) width for such as image (2-dim.) data.
  */
...
// note: implementing this with some kind of "count leading zeros" assembly is a
// big performance win
...
static inline uint32_t ALWAYS_INLINE read32bit(uint8_t *buffer) {
  // embedded CPUs typically can't read unaligned 32-bit words so just read the
  // bytes
  uint32_t value;
...
static inline uint32_t ALWAYS_INLINE getstreambits(uint8_t *in,
  ...
  // a shift of >= "the number of bits in the type of the value being shifted"
  // results in undefined behavior so don't try to shift by 32
  if (numbits != (sizeof(result) * 8)) result &= ~(0xfffffffful << numbits);
...
int32_t dyn_decomp(AGParamRecPtr params, BitBuffer *bitstream, int32_t *pc,
  ...
  RequireAction((bitstream != nil) && (pc != nil) && (outNumBits != nil),
