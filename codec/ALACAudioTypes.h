...
// These are the Channel Layout Tags used in the Channel Layout Info portion of
...
enum {
  ...
  kALACChannelLayoutTag_MPEG_7_1_B =
      (127 << 16) | 8  // C Lc Rc L R Ls Rs LFE    (doc: IS-13818-7 MPEG2-AAC)
};

// ALAC currently only utilizes these channels layouts. There is a one for one
// correspondance between a given number of channels and one of these layout
// tags
static const ALACChannelLayoutTag ALACChannelLayoutTags[kALACMaxChannels] = {
    ...
    kALACChannelLayoutTag_MPEG_7_1_B   // C Lc Rc L R Ls Rs LFE    (doc:
                                       // IS-13818-7 MPEG2-AAC)
};

// AudioChannelLayout from CoreAudioTypes.h. We never need the
// AudioChannelDescription so we remove it
...
// note: this struct is wrapped in an 'alac' atom in the sample description
// extension area note: in QT movies, it will be further wrapped in a 'wave'
// atom surrounded by 'frma' and 'term' atoms
