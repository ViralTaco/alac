...
void init_coefs(int16_t* coefs, uint32_t denshift, int32_t numPairs) {
  ...
  coefs[1] = (BINIT * den) >> 4;
  coefs[2] = (CINIT * den) >> 4;
  for (k = 3; k < numPairs; k++) coefs[k] = 0;
}

void copy_coefs(int16_t* srcCoefs, int16_t* dstCoefs, int32_t numPairs) {
  int32_t k;

  for (k = 0; k < numPairs; k++) dstCoefs[k] = srcCoefs[k];
}
...
void pc_block(int32_t* in, int32_t* pc1, int32_t num, int16_t* coefs,
  ...
  pc1[0] = in[0];
  if (numactive == 0) {
    // just copy if numactive == 0 (but don't bother if in/out pointers the
    // same)
    if ((num > 1) && (in != pc1))
  ...
  if (numactive == 4) {
    ...
    for (j = lim; j < num; j++) {
      ...
      sum1 = 0;
      for (k = 0; k < numactive; k++) sum1 -= coefs[k] * (top - pin[-k]);
