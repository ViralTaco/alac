...
static inline int32_t ALWAYS_INLINE abs_func(int32_t a) {
  // note: the CW PPC intrinsic __abs() turns into these instructions so no need
  // to try and use it
  int32_t isneg = a >> 31;
...
static inline int32_t dyn_code_32bit(int32_t maxbits, uint32_t m, uint32_t k,
  ...
  if (div < MAX_PREFIX_32) {
    ...
    numBits = div + k + 1 - de;
    value = (((1 << div) - 1) << (numBits - div)) + mod + 1 - de;
    if (numBits > 25) goto codeasescape;
  } else {
  codeasescape:
...
static inline void ALWAYS_INLINE dyn_jam_noDeref(unsigned char *out,
  ...
  mask = ~0u >> (32 - numBits);  // mask must be created in two steps to avoid
                                 // compiler sequencing ambiguity
  mask <<= shift;
...
static inline void ALWAYS_INLINE dyn_jam_noDeref_large(unsigned char *out,
  ...
  if (shiftvalue < 0) {
    ...
    *tailptr = (uint8_t)tailbyte;
    ...
    mask = ~0u >> (32 - numBits);
    mask <<= shiftvalue;  // mask must be created in two steps to avoid compiler
