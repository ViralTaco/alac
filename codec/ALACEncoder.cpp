...
// Note: in C you can't typecast to a 2-dimensional array pointer but that's
// what we need when picking which coefs to use so we declare this typedef b/c
// we *can* typecast to this type
...
/*
        Map Format: 3-bit field per channel which is the same as the "element
   tag" that should be placed at the beginning of the frame for that channel.
   Indicates whether SCE, CPE, or LFE. Each particular field is accessed via the
   current channel index.  Note that the channel index increments by two for
   channel pairs.

        For example:

                        C L R 3-channel input		= (ID_CPE << 3) |
   (ID_SCE) index 0 value = (map & (0x7ul << (0 * 3))) >> (0 * 3) index 1 value
   = (map & (0x7ul << (1 * 3))) >> (1 * 3)

                        C L R Ls Rs LFE 5.1-channel input = (ID_LFE << 15) |
   (ID_CPE << 9) | (ID_CPE << 3) | (ID_SCE) index 0 value = (map & (0x7ul << (0
   * 3))) >> (0 * 3) index 1 value = (map & (0x7ul << (1 * 3))) >> (1 * 3) index
   3 value = (map & (0x7ul << (3 * 3))) >> (3 * 3) index 5 value = (map & (0x7ul
   << (5 * 3))) >> (5 * 3) index 7 value = (map & (0x7ul << (7 * 3))) >> (7 * 3)
*/
...
/*
        ...
        For every segment we adopt the following header:
                        ...
                        1 byte mixRes			(middle-side parameter,
   interpreted as signed char)

                        1 byte shiftU			(4 bits modeU, 4 bits
   denShiftU) 1 byte filterU			(3 bits pbFactorU, 5 bits numU)
                        (numU) shorts			(signed DP coefficients
   for V channel)
                             ---Next, 2nd-channel ALAC parameters in case of
   stereo mode--- 1 byte shiftV			(4 bits modeV, 4 bits denShiftV)
                        1 byte filterV			(3 bits pbFactorV, 5
   bits numV) (numV) shorts			(signed DP coefficients for V
   channel)
                             ---After this come the shift-off bytes for (>=
   24)-bit data (n-byte shift) if indicated---
        ...
        -----
                ...
                The presence of certain flag bits changes the header format such
   that the parameters might not even be sent.  The currently defined flags
   format is:

                        0000psse

                        where		0 	= reserved, must be 0
                                                p	= 1-bit field "partial
   frame" flag indicating 32-bit frame length follows this byte ss	= 2-bit
   field indicating "number of shift-off bytes ignored by compression"
                                                e	= 1-bit field indicating
   "escape"

                The "partial frame" flag means that the following segment is not
   equal to the frame length specified in the out-of-band decoder configuration.
   This allows the decoder to deal with end-of-file partial segments without
   incurring the 32-bit overhead for each segment.

                The "shift-off" field indicates the number of bytes at the
   bottom of the word that were passed through uncompressed.  The reason for
   this is that the entropy inherent in the LS bytes of >= 24-bit words quite
   often means that the frame would have to be "escaped" b/c the compressed size
   would be >= the uncompressed size.  However, by shifting the input values
   down and running the remaining bits through the normal compression algorithm,
   a net win can be achieved.  If this field is non-zero, it means that the
   shifted-off bytes follow after the parameter section of the header and before
   the compressed bitstream.  Note that doing this also allows us to use
   matrixing on 32-bit inputs after one or more bytes are shifted off the bottom
   which helps the eventual compression ratio.  For stereo channels, the shifted
   off bytes are interleaved.

        The "escape" flag means that this segment was not compressed b/c the
   compressed size would be
        >= uncompressed size.  In that case, the audio data was passed through
   uncompressed after the header. The other header parameter bytes will not be
   sent.
                ...
                If the segment is not a partial or escape segment, the total
   header size (in bytes) is given exactly by:
        ...
        To generalize middle-side, there are various mixing modes including
   middle-side, each lossless, as embodied in the mix() and unmix() functions.
   These functions exploit a generalized middle-side transformation:
...
*/
int32_t ALACEncoder::EncodeStereo(BitBuffer *bitstream, void *inputBuffer,
                                  uint32_t stride, uint32_t channelIndex,
                                  uint32_t numSamples) {
  BitBuffer workBits;
  BitBuffer startBits =
      *bitstream;  // squirrel away copy of current state in case we need to go
                   // back and do an escape packet
  AGParamRec agParams;
  ...
  RequireAction((mBitDepth == 16) || (mBitDepth == 20) || (mBitDepth == 24) ||
                    (mBitDepth == 32),
                return kALAC_ParamError;);

  // reload coefs pointers for this channel pair
  // - note that, while you might think they should be re-initialized per block,
  // retaining state across blocks
  //	 actually results in better overall compression
  // - strangely, re-using the same coefs for the different passes of the
  // "mixRes" search loop instead of using
  //	 different coefs for the different passes of "mixRes" results in even
  //better compression
  coefsU = (SearchCoefs)mCoefsU[channelIndex];
  coefsV = (SearchCoefs)mCoefsV[channelIndex];

  // matrix encoding adds an extra bit but 32-bit inputs cannot be matrixed b/c
  // 33 is too many so enable 16-bit "shift off" and encode in 17-bit mode
  ...
  for (mixRes = 0; mixRes <= maxRes; mixRes++) {
    ...
    switch (mBitDepth) {
      case 16:
        mix16((int16_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
              numSamples / dilate, mixBits, mixRes);
        break;
      case 20:
        mix20((uint8_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
              numSamples / dilate, mixBits, mixRes);
        break;
      case 24:
        // includes extraction of shifted-off bytes
        mix24((uint8_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
              numSamples / dilate, mixBits, mixRes, mShiftBufferUV,
              bytesShifted);
        break;
      case 32:
        // includes extraction of shifted-off bytes
        mix32((int32_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
              numSamples / dilate, mixBits, mixRes, mShiftBufferUV,
              bytesShifted);
        break;
    }
    ...
    pc_block(mMixBufferU, mPredictorU, numSamples / dilate, coefsU[numU - 1],
             numU, chanBits, DENSHIFT_DEFAULT);
    pc_block(mMixBufferV, mPredictorV, numSamples / dilate, coefsV[numV - 1],
             numV, chanBits, DENSHIFT_DEFAULT);

    // run the lossless compressor on each channel
    set_ag_params(&agParams, MB0, (pbFactor * PB0) / 4, KB0,
                  numSamples / dilate, numSamples / dilate, MAX_RUN_DEFAULT);
    status = dyn_comp(&agParams, mPredictorU, &workBits, numSamples / dilate,
                      chanBits, &bits1);
    RequireNoErr(status, goto Exit;);

    set_ag_params(&agParams, MB0, (pbFactor * PB0) / 4, KB0,
                  numSamples / dilate, numSamples / dilate, MAX_RUN_DEFAULT);
    status = dyn_comp(&agParams, mPredictorV, &workBits, numSamples / dilate,
                      chanBits, &bits2);
    RequireNoErr(status, goto Exit;);
  ...
  switch (mBitDepth) {
    ...
    case 24:
      ...
      mix24((uint8_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
            numSamples, mixBits, mixRes, mShiftBufferUV, bytesShifted);
      break;
    case 32:
      // also extracts the shifted off bytes into the shift buffers
      mix32((int32_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
  ...
  for (uint32_t numUV = kMinUV; numUV <= kMaxUV; numUV += 4) {
    ...
    for (uint32_t converge = 0; converge < 8; converge++) {
      pc_block(mMixBufferU, mPredictorU, numSamples / dilate, coefsU[numUV - 1],
               numUV, chanBits, DENSHIFT_DEFAULT);
      pc_block(mMixBufferV, mPredictorV, numSamples / dilate, coefsV[numUV - 1],
               numUV, chanBits, DENSHIFT_DEFAULT);
    }
  ...
  // test for escape hatch if best calculated compressed size turns out to be
  // more than the input size
  minBits = minBits1 + minBits2 + (8 /* mixRes/maxRes/etc. */ * 8) +
            ((partialFrame == true) ? 32 : 0);
  if (bytesShifted != 0) minBits += (numSamples * (bytesShifted * 8) * 2);

  escapeBits = (numSamples * mBitDepth * 2) +
               ((partialFrame == true) ? 32 : 0) +
  ...
  if (doEscape == false) {
    ...
    if (partialFrame) BitBufferWrite(bitstream, numSamples, 32);
    ...
    if (bytesShifted != 0) {
      ...
      for (index = 0; index < (numSamples * 2); index += 2) {
        ...
        shiftedVal = ((uint32_t)mShiftBufferUV[index + 0] << bitShift) |
                     (uint32_t)mShiftBufferUV[index + 1];
        BitBufferWrite(bitstream, shiftedVal, bitShift * 2);
      }
    }

    // run the dynamic predictor and lossless compression for the "left" channel
    // - note: to avoid allocating more buffers, we're mixing and matching
    // between the available buffers instead
    //		   of only using "U" buffers for the U-channel and "V" buffers for the
    //V-channel
    if (mode == 0) {
      pc_block(mMixBufferU, mPredictorU, numSamples, coefsU[numU - 1], numU,
               chanBits, DENSHIFT_DEFAULT);
    } else {
      pc_block(mMixBufferU, mPredictorV, numSamples, coefsU[numU - 1], numU,
               chanBits, DENSHIFT_DEFAULT);
    ...
    status = dyn_comp(&agParams, mPredictorU, bitstream, numSamples, chanBits,
                      &bits1);
    RequireNoErr(status, goto Exit;);

    // run the dynamic predictor and lossless compression for the "right"
    // channel
    if (mode == 0) {
      pc_block(mMixBufferV, mPredictorV, numSamples, coefsV[numV - 1], numV,
               chanBits, DENSHIFT_DEFAULT);
    } else {
      pc_block(mMixBufferV, mPredictorU, numSamples, coefsV[numV - 1], numV,
               chanBits, DENSHIFT_DEFAULT);
    ...
    set_ag_params(&agParams, MB0, (pbFactor * PB0) / 4, KB0, numSamples,
                  numSamples, MAX_RUN_DEFAULT);
    status = dyn_comp(&agParams, mPredictorV, bitstream, numSamples, chanBits,
                      &bits2);
    RequireNoErr(status, goto Exit;);

    /*	if we happened to create a compressed packet that was actually bigger
       than an escape packet would be, chuck it and do an escape packet
    */
    minBits =
  ...
  if (doEscape == true) {
    /* escape */
    status =
        this->EncodeStereoEscape(bitstream, inputBuffer, stride, numSamples);
...
/*
        EncodeStereoFast()
        - encode a channel pair without the search loop for maximum possible
   speed
*/
int32_t ALACEncoder::EncodeStereoFast(BitBuffer *bitstream, void *inputBuffer,
                                      uint32_t stride, uint32_t channelIndex,
                                      uint32_t numSamples) {
  BitBuffer startBits = *bitstream;  // squirrel away current bit position in
                                     // case we decide to use escape hatch
  AGParamRec agParams;
  uint32_t bits1, bits2;
  ...
  /* speculatively write the bitstream assuming the compressed version will be
   * smaller */
  ...
  // run the dynamic predictor and lossless compression for the "left" channel
  // - note: we always use mode 0 in the "fast" path so we don't need the code
  // for mode != 0
  pc_block(mMixBufferU, mPredictorU, numSamples, coefsU[numU - 1], numU,
           chanBits, DENSHIFT_DEFAULT);
  ...
  set_ag_params(&agParams, MB0, (pbFactor * PB0) / 4, KB0, numSamples,
                numSamples, MAX_RUN_DEFAULT);
  status =
      dyn_comp(&agParams, mPredictorU, bitstream, numSamples, chanBits, &bits1);
  RequireNoErr(status, goto Exit;);
  ...
  set_ag_params(&agParams, MB0, (pbFactor * PB0) / 4, KB0, numSamples,
                numSamples, MAX_RUN_DEFAULT);
  status =
      dyn_comp(&agParams, mPredictorV, bitstream, numSamples, chanBits, &bits2);
  RequireNoErr(status, goto Exit;);
  ...
  if (doEscape == true) {
    ...
    // reset bitstream position since we speculatively wrote the compressed
    // version
    *bitstream = startBits;
...
*/
int32_t ALACEncoder::EncodeStereoEscape(BitBuffer *bitstream, void *inputBuffer,
                                        uint32_t stride, uint32_t numSamples) {
  int16_t *input16;
  int32_t *input32;
  uint8_t partialFrame;
  uint32_t index;
  ...
  BitBufferWrite(bitstream, 0, 12);
  BitBufferWrite(bitstream, (partialFrame << 3) | 1,
                 4);  // LSB = 1 means "frame not compressed"
  if (partialFrame) BitBufferWrite(bitstream, numSamples, 32);
  ...
  switch (mBitDepth) {
    case 16:
      ...
      break;
    case 20:
      // mix20() with mixres param = 0 means de-interleave so use it to simplify
      // things
      mix20((uint8_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
            numSamples, 0, 0);
      ...
      break;
    case 24:
      // mix24() with mixres param = 0 means de-interleave so use it to simplify
      // things
      mix24((uint8_t *)inputBuffer, stride, mMixBufferU, mMixBufferV,
            numSamples, 0, 0, mShiftBufferUV, 0);
      for (index = 0; index < numSamples; index++) {
...
*/
int32_t ALACEncoder::EncodeMono(BitBuffer *bitstream, void *inputBuffer,
                                uint32_t stride, uint32_t channelIndex,
                                uint32_t numSamples) {
  BitBuffer startBits =
      *bitstream;  // squirrel away copy of current state in case we need to go
  ...
  switch (mBitDepth) {
    ...
    case 20:
      ...
      copy20ToPredictor((uint8_t *)inputBuffer, stride, mMixBufferU,
                        numSamples);
      break;
    case 24:
      // convert 24-bit data to 32-bit for the predictor and extract the shifted
      // off byte(s)
      copy24ToPredictor((uint8_t *)inputBuffer, stride, mMixBufferU,
                        numSamples);
      for (index = 0; index < numSamples; index++) {
      ...
      break;
    case 32: {
      // just copy the 32-bit input data for the predictor and extract the
      // shifted off byte(s)
      input32 = (int32_t *)inputBuffer;

      for (index = 0, index2 = 0; index < numSamples;
           index++, index2 += stride) {
  ...
  // brute-force encode optimization loop (implied "encode depth" of 0 if
  // comparing to cmd line tool)
  // - run over variations of the encoding params to find the best choice
  minU = 4;
  maxU = 8;
  minBits = 1ul << 31;
  ...
  if (bytesShifted != 0) minBits += (numSamples * (bytesShifted * 8));

  escapeBits = (numSamples * mBitDepth) + ((partialFrame == true) ? 32 : 0) +
               (2 * 8); /* 2 common header bytes */
...
*/
int32_t ALACEncoder::Encode(AudioFormatDescription theInputFormat,
  ...
  if (theInputFormat.mChannelsPerFrame == 2) {
    // add 3-bit frame start tag ID_CPE = channel pair & 4-bit element instance
    // tag = 0
    ...
    if (mFastMode == false)
      ...
      status =
          this->EncodeStereoFast(&bitstream, theReadBuffer, 2, 0, numFrames);
    RequireNoErr(status, goto Exit;);
    ...
    for (channelIndex = 0; channelIndex < theInputFormat.mChannelsPerFrame;) {
      tag = (sChannelMaps[theInputFormat.mChannelsPerFrame - 1] &
      ...
      switch (tag) {
        case ID_SCE:
          ...
          status = this->EncodeMono(&bitstream, inputBuffer,
                                    theInputFormat.mChannelsPerFrame,
                                    channelIndex, numFrames);
        ...
        case ID_CPE:
          ...
          status = this->EncodeStereo(&bitstream, inputBuffer,
                                      theInputFormat.mChannelsPerFrame,
                                      channelIndex, numFrames);
...
#if VERBOSE_DEBUG
  {
    // if there is room left in the output buffer, add some random fill data to
    // test decoder
    int32_t bitsLeft;
    int32_t bytesLeft;
  ...
  // all good, let iTunes know what happened and remember the total number of
  // input sample frames
...
int32_t ALACEncoder::Finish() {
  /*	// finalize bit rate statistics
          if ( mSampleSize.numEntries != 0 )
                  mAvgBitRate = (uint32_t)( (((float)mTotalBytesGenerated
     * 8.0f) / (float)mSampleSize.numEntries) * ((float)mSampleRate /
     (float)mFrameSize) ); else mAvgBitRate = 0;
  */
  return ALAC_noErr;
}
...
void ALACEncoder::GetMagicCookie(void *outCookie, uint32_t *ioSize) {
  ...
  if (theConfig.numChannels > 2) {
    theChannelLayout.mChannelLayoutTag =
...
int32_t ALACEncoder::InitializeEncoder(AudioFormatDescription theOutputFormat) {
  ...
  // - note: mFrameSize is set in the constructor or via SetFrameSize() which
  // must be called before this routine
  for (uint32_t index = 0; index < kALACMaxChannels; index++)
    mLastMixRes[index] = kDefaultMixRes;

  // the maximum output frame size can be no bigger than (samplesPerBlock *
  // numChannels * ((10 + sampleSize)/8) + 1) but note that this can be bigger
  // than the input size!
  // - since we don't yet know what our input format will be, use our max
  // allowed sample size in the calculation
  ...
  // initialize coefs arrays once b/c retaining state across blocks actually
  // improves the encode ratio
...
*/
void ALACEncoder::GetSourceFormat(const AudioFormatDescription *source,
                                  AudioFormatDescription *output) {
  // default is 16-bit native endian
  // - note: for float input we assume that's coming from one of our decoders
  // (mp3, aac) so it only makes sense
  //		   to encode to 16-bit since the source was lossy in the first
  //place
  // - note: if not a supported bit depth, find the closest supported bit depth
  // to the input one
  ...
  // we support 16/20/24/32-bit integer data at any sample rate and our target
  // number of channels and sample rate were specified when we were configured
  /*
MakeUncompressedAudioFormat( mNumChannels, (float) mOutputSampleRate, mBitDepth,
kAudioFormatFlagsNativeIntegerPacked, output );
*/
}
...
static void AddFiller(BitBuffer *bits, int32_t numBytes) {
  ...
  // out of lameness, subtract 6 bytes to deal with header + alignment as
  // required for fill/data elements
  numBytes -= 6;
  ...
  if (tag == ID_FIL) {
    ...
    // fill element = 4-bit size unless >= 15 then 4-bit size + 8-bit extension
    // size
    if (numBytes >= 15) {
      ...
      // 8-bit extension count field is "extra + 1" which is weird but I didn't
      // define the syntax
      // - otherwise, there's no way to represent 15
      // - for example, to really mean 15 bytes you must encode extensionSize =
      // 1
    ...
    BitBufferWrite(bits, 0x10, 8);  // extension_type = FILL_DATA = b0001 or'ed
                                    // with fill_nibble = b0000
    ...
    // data stream element = 8-bit size unless >= 255 then 8-bit size + 8-bit
    // size
    if (numBytes >= 255) {
    ...
    for (index = 0; index < numBytes; index++) BitBufferWrite(bits, 0x5a, 8);
  }
}
