...
ALACDecoder::ALACDecoder()
    : mMixBufferU(nil), mMixBufferV(nil), mPredictor(nil), mShiftBuffer(nil) {
...
int32_t ALACDecoder::Init(void *inMagicCookie, uint32_t inMagicCookieSize) {
  ...
  // For historical reasons the decoder needs to be resilient to magic cookies
  // vended by older encoders. As specified in the
  // ALACMagicCookieDescription.txt document, there may be additional data
  // encapsulating the ALACSpecificConfig. This would consist of format ('frma')
  // and 'alac' atoms which precede the ALACSpecificConfig. See
  // ALACMagicCookieDescription.txt for additional documentation concerning the
  // 'magic cookie'

  // skip format ('frma') atom if present
  if (theActualCookie[4] == 'f' && theActualCookie[5] == 'r' &&
      theActualCookie[6] == 'm' && theActualCookie[7] == 'a') {
    theActualCookie += 12;
    theCookieBytesRemaining -= 12;
  }

  // skip 'alac' atom header if present
  if (theActualCookie[4] == 'a' && theActualCookie[5] == 'l' &&
      theActualCookie[6] == 'a' && theActualCookie[7] == 'c') {
    theActualCookie += 12;
    theCookieBytesRemaining -= 12;
  }
  ...
  if (theCookieBytesRemaining >= sizeof(ALACSpecificConfig)) {
    theConfig.frameLength =
        Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->frameLength);
    theConfig.compatibleVersion =
        ((ALACSpecificConfig *)theActualCookie)->compatibleVersion;
    ...
    theConfig.maxRun =
        Swap16BtoN(((ALACSpecificConfig *)theActualCookie)->maxRun);
    theConfig.maxFrameBytes =
        Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->maxFrameBytes);
    theConfig.avgBitRate =
        Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->avgBitRate);
    theConfig.sampleRate =
        Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->sampleRate);

    mConfig = theConfig;

    RequireAction(mConfig.compatibleVersion <= kALACVersion,
  ...
  // Currently, the Channel Layout Info portion of the magic cookie (as defined
  // in the ALACMagicCookieDescription.txt document) is unused by the decoder.
...
/*
        Decode()
        - the decoded samples are interleaved into the output buffer in the
   order they arrive in the bitstream
*/
int32_t ALACDecoder::Decode(BitBuffer *bits, uint8_t *sampleBuffer,
  ...
  int16_t coefsU[32];  // max possible size is 32 although NUMCOEPAIRS is the
                       // current limit
  int16_t coefsV[32];
  uint8_t numU, numV;
  uint8_t mixBits;
  ...
  while (status == ALAC_noErr) {
    ...
    switch (tag) {
      ...
      case ID_LFE: {
        ...
        // read the 1-bit "partial frame" flag, 2-bit "shift-off" flag & 1-bit
        // "escape" flag
        ...
        if (escapeFlag == 0) {
          ...
          // Assert( (mixBits == 0) && (mixRes == 0) );		// no mixing for
          // mono
          ...
          // if shift active, skip the the shift buffer but remember where it
          // starts
          if (bytesShifted != 0) {
            shiftBits = *bits;
          ...
          // decompress
          set_ag_params(&agParams, mConfig.mb, (pb * pbFactorU) / 4, mConfig.kb,
                        numSamples, numSamples, mConfig.maxRun);
          status = dyn_decomp(&agParams, bits, mPredictor, numSamples, chanBits,
                              &bits1);
          RequireNoErr(status, goto Exit;);

          if (modeU == 0) {
            unpc_block(mPredictor, mMixBufferU, numSamples, &coefsU[0], numU,
                       chanBits, denShiftU);
          } else {
            // the special "numActive == 31" mode can be done in-place
            unpc_block(mPredictor, mPredictor, numSamples, nil, 31, chanBits,
                       0);
            unpc_block(mPredictor, mMixBufferU, numSamples, &coefsU[0], numU,
                       chanBits, denShiftU);
          }
          ...
          // uncompressed frame, copy data into the mix buffer to use common
          // output code
          shift = 32 - chanBits;
          if (chanBits <= 16) {
            ...
            // BitBufferRead() can't read more than 16 bits at a time so break
            // up the reads
        ...
        if (bytesShifted != 0) {
          ...
          for (i = 0; i < numSamples; i++)
            mShiftBuffer[i] =
                (uint16_t)BitBufferRead(&shiftBits, (uint8_t)shift);
        }
        ...
        switch (mConfig.bitDepth) {
          ...
          case 24:
            ...
            if (bytesShifted != 0)
              copyPredictorTo24Shift(mMixBufferU, mShiftBuffer, out24,
                                     numChannels, numSamples, bytesShifted);
            else
          ...
          case 32:
            ...
            if (bytesShifted != 0)
              copyPredictorTo32Shift(mMixBufferU, mShiftBuffer, out32,
                                     numChannels, numSamples, bytesShifted);
            else
      ...
      case ID_CPE: {
        ...
        if (escapeFlag == 0) {
          ...
          // if shift active, skip the interleaved shifted values but remember
          // where they start
          if (bytesShifted != 0) {
            shiftBits = *bits;
          ...
          set_ag_params(&agParams, mConfig.mb, (pb * pbFactorV) / 4, mConfig.kb,
                        numSamples, numSamples, mConfig.maxRun);
          status = dyn_decomp(&agParams, bits, mPredictor, numSamples, chanBits,
                              &bits2);
          RequireNoErr(status, goto Exit;);
          ...
          if (modeV == 0) {
            unpc_block(mPredictor, mMixBufferV, numSamples, &coefsV[0], numV,
                       chanBits, denShiftV);
          } else {
            // the special "numActive == 31" mode can be done in-place
            unpc_block(mPredictor, mPredictor, numSamples, nil, 31, chanBits,
          ...
          // uncompressed frame, copy data into the mix buffers to use common
          // output code
        ...
        // - note that mixRes = 0 means just interleave so we use that path for
        // uncompressed frames
        switch (mConfig.bitDepth) {
          case 16:
            out16 = &((int16_t *)sampleBuffer)[channelIndex];
            unmix16(mMixBufferU, mMixBufferV, out16, numChannels, numSamples,
                    mixBits, mixRes);
            break;
          case 20:
            out20 = (uint8_t *)sampleBuffer + (channelIndex * 3);
            unmix20(mMixBufferU, mMixBufferV, out20, numChannels, numSamples,
                    mixBits, mixRes);
            break;
          case 24:
            out24 = (uint8_t *)sampleBuffer + (channelIndex * 3);
...
#if !DEBUG
    // if we've decoded all of our channels, bail (but not in debug b/c we want
    // to know if we're seeing bad bits)
    // - this also protects us if the config does not match the bitstream or
    // crap data bits follow the audio bits
...
NoMoreChannels:
  ...
  // if we get here and haven't decoded all of the requested channels, fill the
...
int32_t ALACDecoder::FillElement(BitBuffer *bits) {
  ...
  if (count == 15) count += (int16_t)BitBufferReadSmall(bits, 8) - 1;
...
int32_t ALACDecoder::DataStreamElement(BitBuffer *bits) {
  ...
  if (count == 255) count += BitBufferReadSmall(bits, 8);

  // the align flag means the bitstream should be byte-aligned before reading
  // the following data bytes
  if (data_byte_align_flag) BitBufferByteAlign(bits, false);
...
/*
        ZeroN()
        - helper routines to clear out output channel buffers when decoding
   fewer channels than requested
*/
static void Zero16(int16_t *buffer, uint32_t numItems, uint32_t stride) {
  if (stride == 1) {
...
static void Zero24(uint8_t *buffer, uint32_t numItems, uint32_t stride) {
  if (stride == 1) {
    ...
    for (uint32_t index = 0; index < (numItems * stride * 3);
         index += (stride * 3)) {
      buffer[index + 0] = 0;
      buffer[index + 1] = 0;
      buffer[index + 2] = 0;
