...
// 20-bit routines
// - the 20 bits of data are left-justified in 3 bytes of storage but
// right-aligned for input/output predictor buffers
...
// 24-bit routines
// - the 24 bits of data are right-justified in the input/output predictor
// buffers
...
// 32-bit routines
// - note that these really expect the internal data width to be < 32 but the
// arrays are 32-bit
...
// 20/24-bit <-> 32-bit helper routines (not really matrixing but convenient to
// put here)
...
void copyPredictorTo20(int32_t* in, uint8_t* out, uint32_t stride,
  ...
  // 32-bit predictor values are right-aligned but 20-bit output values should
  // be left-aligned in the 24-bit output buffer
  for (j = 0; j < numSamples; j++) {
    int32_t val = in[j];
...
void copyPredictorTo32(int32_t* in, int32_t* out, uint32_t stride,
  ...
  // this is only a subroutine to abstract the "iPod can only output 16-bit
  // data" problem
  for (i = 0, j = 0; i < numSamples; i++, j += stride) out[j] = in[i];
}
